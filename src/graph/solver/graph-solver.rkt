...
(define (graph-solver-bfs graph level root-node-id goal-node-id)
  (graph-solver graph level root-node-id goal-node-id O0:bfs void void O3:bfs))
(define (graph-solver-dfs graph level root-node-id goal-node-id)
  (graph-solver graph level root-node-id goal-node-id O0:dfs void void O3:dfs))
(define (graph-solver-dijkstra graph level root-node-id goal-node-id)
  (graph-solver graph level root-node-id goal-node-id O0:dijkstra void void O3:dijkstra))
(define (graph-solver-a-star graph level root-node-id goal-node-id)
  (graph-solver graph level root-node-id goal-node-id O0:a-star void void O3:a-star))
